...
class HomeController extends Controller
{
    ...
    public function getTotals()
    {
        if (request()->ajax()) {
            ...
            $gp = Variation::select('*')->join("purchase_lines", "variations.product_id", "=", "purchase_lines.product_id")->get();
            $gp_total = 0;
            for($a=0; $a<count($gp); $a++){
                $gp_total = $gp_total + (($gp[$a]->sell_price_inc_tax - $gp[$a]->dpp_inc_tax) * ($gp[$a]->quantity - $gp[$a]->quantity_sold));
            }
            $output['gp_total'] = $gp_total;
            return $output;
        }
    }
