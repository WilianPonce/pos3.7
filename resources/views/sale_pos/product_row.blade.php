<tr class="product_row" data-row_index="{{$row_count}}">
	...
	<td class="{{$hide_tax}}">
		...
		<input type="text" name="products[{{$row_count}}][unit_price_inc_tax1]" class="form-control pos_unit_price_inc_tax1 input_number" value="{{@num_format($product->tax_amount)}}">
	</td>
	<td class="text-center v-center">
		...
		@endphp
		<input type="{{$subtotal_type}}" class="form-control pos_line_total @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="{{ number_format(ceil($product->quantity_ordered*$unit_price_inc_tax )) }}">
		<span class="display_currency pos_line_total_text @if(!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif" data-currency_symbol="true">{{ number_format(ceil($product->quantity_ordered*$unit_price_inc_tax)) }}</span>
	</td>
	<td class="text-center v-center">
		@php
			$subtotal_type = !empty($pos_settings['is_pos_subtotal_editable']) ? 'text' : 'hidden';
		@endphp
		<input type="{{$subtotal_type}}" class="form-control pos_line_total1 @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="{{ number_format(ceil($unit_price_inc_tax * (1+($product->tax_amount/100))) * $product->quantity_ordered) }}">
		<span class="display_currency pos_line_total_text1 @if(!empty($pos_settings['is_pos_subtotal_editable'])) hide @endif" data-currency_symbol="true">{{ number_format(ceil($unit_price_inc_tax * (1+($product->tax_amount/100))) * $product->quantity_ordered) }}</span>
	</td>
